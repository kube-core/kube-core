...
log_debug "${project_name} - Configure Minio"

minioUser=${1:-"minioUser"}
minioPassword=${2:-"minioPassword"}
bucketsList=${3:-"test-bucket"}
envs=${4:-"development"}

baseDomain=${cluster_config_domain}

export MC_HOST_${project_name}=https://${minioUser}:${minioPassword}@minio.minio.${baseDomain}
mc ls ${project_name}

for bucket in ${bucketsList}; do
    mc mb ${project_name}/${bucket} || true
done

for env in envs; do
    mc mb ${project_name}/${env}-default-bucket || true
    mc policy set public ${project_name}/${env}-default-bucket || true
done
