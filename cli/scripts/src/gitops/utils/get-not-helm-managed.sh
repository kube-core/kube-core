...
## Header End
## Docs Start ##
## Gets a list of gitops manifests that have no helm labels/annotations
## Docs End ##


log_debug "${project_name} - Getting not helm managed files"
...
command=$(echo "grep -r -l '${query}' config")
eval "${command}" | xargs grep -LE 'managed-by: Helm|heritage: Helm|helm\.sh\/hook'
