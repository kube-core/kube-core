...
## Header End
## Docs Start ##
## Prints: Reads local values and stream then as JSON lines
## Docs End ##

localValuesPath=${1:-"${local_path}/values"}
mkdir -p $localValuesPath

# Sorted by length and alphabetically so the merge sequence is predictable
fileList=$(find $localValuesPath -type f | awk '{ print length, $0 }' | sort -n | awk '{print $2}')
shortPathFileList=$(find $localValuesPath -type f | sed "s|${localValuesPath}/||")
