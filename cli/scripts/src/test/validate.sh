...
## Header End
## Docs Start ##
## Validates all gitops manifests using kubeval
## Docs End ##

log_debug "${project_name} - Validating files..."
...
kubevalConfigPath=${test_validate_config_path}
kubevalOutputPath=${testOutputPath}/kubeval
resultsPath=${kubevalOutputPath}/results

# Cleans up
rm -rf ${kubevalOutputPath}
...
# TODO: -o json and filter / jsonl
# Convert to JSONL
# cat "${resultsPath}" | grep -vE 'WARN|ERR' | jq -r 'map(tostring) | reduce .[] as $item (""; . + $item + "\n")' > ${resultsPath}
# cat "${resultsPath}" | grep -v '\"status\":\"valid\"' > ${resultsPath}
