...
kubeAuditConfigPath=${test_audit_config_path}
auditOutputPath=${testOutputPath}/audit
resultsPath=${auditOutputPath}/results

# Auditing

files=$(find ${gitOpsConfigPath} -type f)

# Cleans up
rm -rf ${auditOutputPath}
...
if cat ${resultsPath} | grep -q "error"; then
    echo "ERROR: Errors found while auditing!"
...
# cat ${resultsPath}| \
# node -e 'const rl = readline.createInterface({ input: process.stdin }); !async function () { let idx = 0; for await (const line of rl) { process.stdout.write((++idx === 1 ? "[" : "\n,") + JSON.stringify(JSON.parse(line))); } process.stdout.write("]"); }()' | \
# tee ${resultsPath}| jq 'length'
