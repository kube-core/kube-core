...
## Header End
## Docs Start ##
## Lints all manifests using yamllint
## Docs End ##

log_debug "${project_name} - Linting files..."
...
lintConfigPath=${test_lint_config_path}
lintOutputPath=${testOutputPath}/lint
resultsPath=${lintOutputPath}/results

# Cleans up
rm -rf ${lintOutputPath}
mkdir -p ${lintOutputPath}

yamllint ${gitOpsConfigPath} -c ${lintConfigPath} > ${resultsPath} || true
cat ${resultsPath}
