...
log_debug "${project_name} - Installing external-dns"

installType="${install_type}"

if [[ "${installType}" == "helmfile" ]]
then
    echo "WARNING: This step is done with helmfile !"
    echo "WARNING: Not compatible with GitOps, only use in dev !"
    echo "Using Helmfile to install external-dns"
    cd ${clusterConfigDirPath}
    helmfile -f ${helmfilePath} -l name=external-dns apply
    echo "external-dns installed with helmfile !"
fi


if [[ "${installType}" == "gitops" ]]
then
    echo "Generating sealed-secrets manifests"
    cd ${clusterConfigDirPath}
    ${scripts_gitops_helmfile_path} "-l name=external-dns"
    ${scripts_gitops_utils_generate_namespaces_path}
    echo "external-dns manifests & namespace generated. You can push them to GitOps !"
fi


if [[ "${installType}" == "gitops-ssa" ]]
then
    echo "Generating external-dns manifests"
    cd ${clusterConfigDirPath}
    ${scripts_gitops_helmfile_path} "-l name=external-dns"
    ${scripts_gitops_utils_generate_namespaces_path}
    ...
    echo "kubectl apply -R -f ${config_path}/external-dns --server-side"

    kubectl apply -R -f ${config_path}/external-dns --server-side --dry-run=server

    echo "external-dns manifests & namespace generated. Applied with SSA (dry-run) !"
fi
