...
log_debug "Creating namespace secrets if it doesn't exist..."
kubectl create ${kubectlArgs} ns secrets || true # TODO: More GitOps friendly
# TODO: Decide if the following is better instead (possible conflicts on namespace customisations?)
# kubectl create ns configmaps --dry-run=client -o yaml > ${configmaps_path}/configmaps/namespace.yaml || true
# kubectl create -f ${configmaps_path}/configmaps/namespace.yaml || true
mkdir -p ${secrets_path}

localSealedSecretsPath=${secrets_path}/output
localReplicatedSecretsPath=${secrets_path}/replicated

apply_config "${localSealedSecretsPath}"
apply_config "${localReplicatedSecretsPath}"

log_info "Done Applying local Secrets!"
