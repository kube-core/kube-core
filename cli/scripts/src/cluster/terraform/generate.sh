...
replace_variables() {
    ...
    -e "s/KUBE_CORE_CLUSTER_CONFIG_DOMAIN/${cluster_config_domain}/g" \
    -e "s/KUBE_CORE_CLUSTER_CONFIG_DOMAIN_SNAKECASE/${cluster_config_domain}/g s/\./-/" \
    -e "s/KUBE_CORE_CLUSTER_CONFIG_ADMIN_EMAIL/${cluster_config_adminEmail}/g" \
    -e "s/KUBE_CORE_CLUSTER_CONFIG_NETWORK_MASTER/${cluster_config_network_master}/g" \
    -e "s/KUBE_CORE_CLOUD_PROJECT/${cloud_project}/g" \
    -e "s|KUBE_CORE_CLUSTER_CONFIG_NETWORK_VPC_SUBNET_RANGE|${cluster_config_network_vpc_subnet}|g" \
    -e "s|KUBE_CORE_CLUSTER_CONFIG_NETWORK_VPC_PODS_RANGE|${cluster_config_network_vpc_pods}|g" \
    -e "s|KUBE_CORE_CLUSTER_CONFIG_NETWORK_VPC_SERVICES_RANGE|${cluster_config_network_vpc_services}|g" \
    -e "s|KUBE_CORE_CLUSTER_CONFIG_NETWORK_VPC_CONTROLPLANE_RANGE|${cluster_config_network_vpc_controlplane}|g" \
    -e "s/KUBE_CORE_CLUSTER_CONFIG_NETWORK_CLUSTER_BLOCK/${cluster_config_network_cluster_block}/g" \
    -e "s/KUBE_CORE_CLUSTER_CONFIG_NETWORK_MAX_PODS_PER_NODE/${cluster_config_network_maxPodsPerNode}/g" \
    -e "s/KUBE_CORE_CLOUD_DEFAULT_LOCATION/${cloud_default_location}/g" \
    -e "s/KUBE_CORE_CLUSTER_SPECS_KUBERNETES_VERSION/${cluster_specs_kubernetes_version}/g" \
...
if [[ ! -z ${tfTemplates} ]]; then
    ...
    mkdir -p ${targetPath}/cluster
    while read template; do
        replace_variables $template > ${targetPath}/cluster/$(basename $template)
    done <<< "${tfTemplates}"
fi
tfTemplates=$(find ${currentScriptPath}/templates/project -type f -name '*.tf')
if [[ ! -z ${tfTemplates} ]]; then
    targetPath=${clusterConfigDirPath}/terraform/${cluster_config_shortName}
    mkdir -p ${targetPath}/project
    while read template; do
        replace_variables $template > ${targetPath}/project/$(basename $template)
    done <<< "${tfTemplates}"
fi
tfTemplates=$(find ${currentScriptPath}/templates/setup -type f -name '*.tf')
if [[ ! -z ${tfTemplates} ]]; then
    targetPath=${clusterConfigDirPath}/terraform/${cluster_config_shortName}
    mkdir -p ${targetPath}/setup
    while read template; do
        replace_variables $template > ${targetPath}/setup/$(basename $template)
    done <<< "${tfTemplates}"
fi
tfTemplates=$(find ${currentScriptPath}/templates/common -type f -name '*.tf')
if [[ ! -z ${tfTemplates} ]]; then
    targetPath=${clusterConfigDirPath}/terraform/${cluster_config_shortName}
    mkdir -p ${targetPath}/cluster
    mkdir -p ${targetPath}/project
