...
# Cluster Config
clusterConfigPath=$(eval find ./$(printf "{$(echo %{1..7}q,)}" | sed 's/ /\.\.\//g') -maxdepth 1 -name cluster-config.yaml | head -n 1 | xargs realpath) || true
...
## Header End
## Docs Start ##
## Gets core helmfile env values. Wrapped by: kube-core values core
## Docs End ##

# Script
args=${1:-""}

helmfile -f ${helmfilePath} build | yq e -N -P "select(di == 0) | .renderedvalues${args}" -
