...
if [ $# -eq 0 ]; then
    ...
    echo "You need to provide arguments: chartName"
    exit
else
    plop --plopfile ${corePath}/cli/plopfile.mjs local-chart ${@} 2> ${tmpReleasesFolder}/.plop.out.log > ${tmpReleasesFolder}/.plop.out.data
fi
...
mkdir -p ${corePath}/releases/local
cd ${corePath}/releases/local
