...
export default class WorkspaceOpen extends BaseCommand {
  ...
  static examples = [`
    ...
    $ kube-core workspace open -n integration # Opens first host on every ingress, in integration Namespace
    $ kube-core workspace open -n integration admin # Opens first host on every ingress, in integration Namespace, matching: -l workspace.kube-core.io/name=admin
  `];
  ...
  async run(): Promise<void> {
    ...
    if (ingressList.items.length > 0) {
      ...
      console.log('No workspace Ingress found. Use the label workspace.kube-core.io/name to create your workspaces.')
    }
  }
}
