...
`,
  ...
  static flags = {
    ...
    classic: Flags.boolean({
      description:
        "Generate environments: integration, validation, preproduction, production",
      hidden: false,
      required: false,
    }),
  };
  ...
  async run(): Promise<void> {
    ...
    if (flags.quickstart === true) {
      files = await this.utils.getFilesAsList(
        upath.join(this.corePath, "core/templates/generate/quickstart")
      );
      ...
      files = await this.utils.getFilesAsList(
        upath.join(this.corePath, "core/templates/generate/classic")
      );
      for (let file of files) {
        let { name, ext } = upath.parse(file);
        let targetPath = upath.join(localDir, `${name}${ext}`);
