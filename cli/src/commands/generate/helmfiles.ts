...
`,
  ...
  static flags = {
    ...
    envs: Flags.string({
      ...
      multiple: true,
      default: [],
      parse: async input => {if (Array.isArray(input)) {return input} else {return input.split(',')}},
    }),
  ...
  async run(): Promise<void> {
    ...
    let envs = undefined;
    let defaultEnvs = ["dev", "integration", "validation", "preproduction", "production"];
    ...
    if (flags.core) {
      ...
      for (let file of files) {
        ...
        writtenFiles.core.push(upath.relative(this.clusterConfigDirPath,targetPath))
      }
    }

    if (flags.lib) {
      const localLibDir = upath.join(this.clusterConfigDirPath, `helmfiles`,`lib`);
      ...
      for (let file of files) {
        ...
        fileData = fileData.replaceAll(
          ...
          [`git::${this.coreRemotePath}@`, 'core', `helmfile.yaml.gotmpl?ref=v${this.config.version}`].join('/')
        );
        fileData = fileData.replaceAll(
        ...
        fileData = fileData.replaceAll(
          ...
          [`git::${this.coreRemotePath}@`, 'core', `cluster.yaml.gotmpl?ref=v${this.config.version}`].join('/')
        );
        fileData = fileData.replaceAll(
        ...
        fileData = fileData.replaceAll(
          ...
          [`git::${this.coreRemotePath}@`, 'core', `applications.yaml.gotmpl?ref=v${this.config.version}`].join('/')
        );
        fileData = fileData.replaceAll(
        ...
        fileData = fileData.replaceAll(
          ...
          [`git::${this.coreRemotePath}@`, 'core', `services.yaml.gotmpl?ref=v${this.config.version}`].join('/')
        );
        await this.utils.mkdir(upath.parse(targetPath).dir);
        await fs.writeFile(targetPath, fileData);
        writtenFiles.lib.push(upath.relative(this.clusterConfigDirPath,targetPath))
      }
    }
    ...
    if ((flags.applications) && (envs)) {
      ...
      for (let file of files) {
        for (let env of envs) {
          let { name, ext } = upath.parse(file);
          let targetPath = upath.join(localDir, `${env}.applications.helmfile.yaml.gotmpl`);
          let fileData = await fs.readFile(file, "utf8");
          fileData = fileData.replaceAll(
            "KUBE_CORE_CLUSTER_CONFIG_CONTEXT",
            this.clusterConfig.cluster.config.context
          ...
          await this.utils.mkdir(upath.parse(targetPath).dir);
          await fs.writeFile(targetPath, fileData);
          writtenFiles.applications.push(upath.relative(this.clusterConfigDirPath,targetPath))
        }
      }
    }
    ...
    if ((flags.services) && (envs)) {
      ...
      for (let file of files) {
        for (let env of envs) {
          let { name, ext } = upath.parse(file);
          let targetPath = upath.join(localDir, `${env}.services.helmfile.yaml.gotmpl`);
          let fileData = await fs.readFile(file, "utf8");
          fileData = fileData.replaceAll(
            "KUBE_CORE_CLUSTER_CONFIG_CONTEXT",
            this.clusterConfig.cluster.config.context
    ...
    let toPush = [...writtenFiles.core, ...writtenFiles.applications, ...writtenFiles.services]
    ...
    await fs.writeFile(upath.join(this.clusterConfigDirPath, "helmfile.yaml"), YAML.stringify(result));
  }
}
