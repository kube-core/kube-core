...
export default class GenerateValues extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    let releaseFiles = await this.utils.getFilesAsList(upath.join(this.corePath, "core/layers/releases"))
    let configFiles = await this.utils.getFilesAsList(upath.join(this.corePath, "core/layers/config"))
    // console.log(files)
    ...
    for(let file of configFiles) {
      ...
      let targetPath = upath.join(localConfigDir, "core/config", `${name}${ext}`)
