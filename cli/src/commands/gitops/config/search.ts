...
export default class GitopsConfigSearch extends BaseCommand {
  ...
  static examples = [`# Search by path in config
  ...
  async run(): Promise<void> {
    ...
    if (this.clusterContext === true) {
        ...
        if(!this.gitopsConfigHasChanges && this.utils.fileExists(current)) {
          ...
          let fullConfig = await this.utils.loadYamlFilesFromPathAsDataObject(this.clusterConfigDirPath, "config")
          ...
          data = await this.utils.cliStream('gron', [], {input: jsonConfig, maxBuffer: 300_000_000})
          if(data.stdout) {
            data = data.stdout
          }
        ...
        if (argv[0]) {
          ...
          const filteredData = await this.utils.cliStream('grep', [filter], {input: data, maxBuffer: 300_000_000})
