...
export default class GitopsConfigSearch extends BaseCommand {
  ...
  static examples = [`# Global gitops diff
  ...
  async run(): Promise<void> {
    ...
    if (this.clusterContext === true) {
        ...
        if(!await this.utils.fileExists(currentIndexPath)) {
          console.warn(`Current index is missing. Follow these instructions: kube-core gitops config index --help`)
          return
        }

        if(!await this.utils.fileExists(nextIndexPath)) {
          console.warn(`Next index is missing. Follow these instructions: kube-core gitops config index --help`)
          return
        }
        ...
        // --include
        if (flags.include) {
          currentFilteredData = await this.utils.cliStream('grep', ["-E", flags.include, `${currentIndexPath}`], {maxBuffer: 900_000_000})
          nextFilteredData = await this.utils.cliStream('grep', ["-E", flags.include, `${nextIndexPath}`], {maxBuffer: 900_000_000})
        } 
        
        // --exclude
        if (flags.exclude) {
          if(currentFilteredData.stdout && nextFilteredData.stdout) {
            currentFilteredData = await this.utils.cliStream('grep', ["-vE", flags.exclude], {input: currentFilteredData.stdout, maxBuffer: 900_000_000})
            nextFilteredData = await this.utils.cliStream('grep', ["-vE", flags.exclude], {input: nextFilteredData.stdout, maxBuffer: 900_000_000})
          } else {
            currentFilteredData = await this.utils.cliStream('grep', ["-vE", flags.exclude, `${currentIndexPath}`], {maxBuffer: 900_000_000})
            nextFilteredData = await this.utils.cliStream('grep', ["-vE", flags.exclude, `${nextIndexPath}`], {maxBuffer: 900_000_000})
          }
        }
        // --filter
        if (flags.filter) {
          ...
          // Ungron config 
          ungronCurrentFilteredData = await this.utils.cliStream('gron', ["--ungron"], {input: currentInput, maxBuffer: 900_000_000})
          ungronNextFilteredData = await this.utils.cliStream('gron', ["--ungron"], {input: nextInput, maxBuffer: 900_000_000})
          
          // Run jq filter
          jqCurrentFilteredData = await this.utils.cliStream('jq', [flags.filter], {input: ungronCurrentFilteredData.stdout, maxBuffer: 900_000_000})
          jqNextFilteredData = await this.utils.cliStream('jq', [flags.filter], {input: ungronNextFilteredData.stdout, maxBuffer: 900_000_000})

          // console.log(jqCurrentFilteredData.stdout)
          // process.exit()

          // Regron config
          regronCurrentFilteredData = await this.utils.cliStream('gron', [], {input: jqCurrentFilteredData.stdout, maxBuffer: 900_000_000})
          regronNextFilteredData = await this.utils.cliStream('gron', [], {input: jqNextFilteredData.stdout, maxBuffer: 900_000_000})
        }

        let currentData
        let nextData

        if(regronCurrentFilteredData && regronNextFilteredData && currentFilteredData && nextFilteredData) {
        ...
        if( (current && currentData && next && nextData) && (current !== currentIndexPath) && (next !== nextIndexPath) ) {
