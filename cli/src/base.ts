...
export default abstract class extends Command {
  ...
  async init() {
    ...
    this.scriptsPath = `${upath.resolve(`${require.main.filename}/../../scripts`)}`;
    this.corePath = `${upath.resolve(`${this.scriptsPath}/../../`)}`;
    let scriptPath = `${upath.join(this.scriptsPath, "src/utils/read-core-path.sh")}`;
    let defaultClusterConfigPath = upath.join(this.scriptsPath, "default-cluster-config.yaml")
    ...
    if (!foundPath) {
      ...
      const gitStatus = await this.utils.cliStream('git', ["status", "--porcelain", upath.join(this.clusterConfigDirPath, "config")], {maxBuffer: 300_000_000})
      if (gitStatus.stdout != "") {
