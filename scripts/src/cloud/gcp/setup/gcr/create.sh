...
serviceAccount="gcr-admin"
credentialsFile="${secrets_path}/input/${serviceAccount}.json"
dockerCredentialsFile="${secrets_path}/manifests/docker-registry-${serviceAccount}.yaml"
mkdir -p "${secrets_path}/manifests"
...
gcloud iam roles create gcrAdmin \
  ...
  --permissions=storage.objects.get,storage.objects.create,storage.objects.list,storage.objects.update,storage.objects.delete,storage.buckets.create,storage.buckets.get,cloudbuild.builds.get,cloudbuild.builds.list || true
...
gcloud projects add-iam-policy-binding ${project_name} \
  ...
  --role=projects/${project_name}/roles/gcrAdmin || true
...
kubectl --namespace default create secret docker-registry docker-registry-${serviceAccount} \
  --docker-server=eu.gcr.io \
  --docker-username=_json_key \
  --docker-password="$(cat ${credentialsFile})" \
  --dry-run \
  -o yaml > ${dockerCredentialsFile}
